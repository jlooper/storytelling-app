<template>
  <div class="chacheli-designer-control">
    ...
    <span class="form" v-show="editMode">
      <b>Add new</b>: text:
      <input v-model="chacheli.text" type="text" @keyup.enter="add" />
      component:
      <select v-model="chacheli.comp">
        <option value="dummy-red">Red</option>
        <option value="dummy-green">Green</option>
        <option value="dummy-blue">Blue</option>
      </select>
      local data:
      <input v-model="localData.content" type="text" @keyup.enter="add" />
