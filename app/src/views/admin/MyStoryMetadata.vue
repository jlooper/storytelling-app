			...
			<div class="column is-one-quarter m-5">
				<aside class="pt-3">
					...
					<div v-if="keyphrases.length > 0">
						<span v-for="tag in keyphrases" :key="tag.id" class="tag is-primary">{{ tag }}</span>
					</div>
					<span class="navbar-item">
						<span>Suggested Organizations</span>
					</span>
					<div v-if="organizations.length > 0">
						<span v-for="tag in organizations" :key="tag.id" class="tag is-info">{{ tag }}</span>
					</div>
					<span class="navbar-item">
						<span>Suggested Locations</span>
					</span>
					<div v-if="locations.length > 0">
						<span v-for="tag in locations" :key="tag.id" class="tag is-warning">{{ tag }}</span>
					</div>
					<span class="navbar-item">
						<span>Suggested People</span>
					</span>
					<div v-if="people.length > 0">
						<span v-for="tag in people" :key="tag.id" class="tag is-link">{{ tag }}</span>
					</div>
				</aside>
			</div>
		</div>
